...
<#
.SYNOPSIS
    Publishes ClaudePopup and optionally deploys to a network update location.

.PARAMETER DeployPath
    Network path to copy the exe and metadata.json to (e.g. \\server\share\ClaudePopup).
...
.EXAMPLE
    .\publish.ps1
    .\publish.ps1 -DeployPath "\\ps1-storage\Install\ClaudePopup" -ReleaseNotes "Added auto-update feature"
#>
param(
    [string]$DeployPath = "",
...
$versionFile = "src\ClaudePopup.Win\Core\AppVersion.cs"
$projectDir  = "src\ClaudePopup.Win"
$releaseDir  = "release"
...
# --- Clean and publish ---
if (Test-Path "$releaseDir\ClaudePopup.exe") {
    Remove-Item "$releaseDir\ClaudePopup.exe" -Force
}
...
if ($DeployPath -and (Test-Path $DeployPath)) {
    ...
    Copy-Item "$releaseDir\ClaudePopup.exe" $DeployPath -Force
...
Write-Host "  ClaudePopup.exe  $('{0:N0}' -f (Get-Item "$releaseDir\ClaudePopup.exe").Length) bytes" -ForegroundColor Gray
