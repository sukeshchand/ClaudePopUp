...
static class Uninstaller
{
    ...
    public static UninstallResult Run(out string? cleanupBatPath)
    {
        ...
        string toolsDir = Path.Combine(userProfile, ".claude", "tools", "ClaudePopup");
        ...
        string toolsExe = Path.Combine(toolsDir, "ClaudePopup.exe");
        ...
        try
        {
            ...
            // Step 2: Remove ClaudePopup hooks from settings.json
            ...
            cleanupBatPath = Path.Combine(Path.GetTempPath(), "claudepopup_uninstall.cmd");
            File.WriteAllText(cleanupBatPath, batLines.ToString(), Encoding.ASCII);

            log.AppendLine();
            log.Append("ClaudePopup has been uninstalled. Please restart any running Claude Code instances.");
    ...
    private static void RemoveHooksFromSettings(string settingsPath)
    {
        ...
        foreach (var eventName in eventNames)
        {
            ...
            for (int i = eventArray.Count - 1; i >= 0; i--)
            {
                ...
                // Remove any hook entries that reference Show-ClaudePopup
                for (int j = hooksArray.Count - 1; j >= 0; j--)
                {
                    string? command = hooksArray[j]?["command"]?.GetValue<string>();
                    if (command != null && command.Contains("Show-ClaudePopup"))
                    {
