...
class SetupForm : Form
{
    ...
    public SetupForm()
    {
        Text = "ClaudePopup - Setup Instructions";
        ...
        _toolsDir = Path.Combine(_userProfile, ".claude", "tools", "ClaudePopup");
        _installExePath = Path.Combine(_toolsDir, "ClaudePopup.exe");
        ...
        _hookCommand = $"powershell.exe -ExecutionPolicy Bypass -File \\\"{hooksDirJsonEscaped}\\\\Show-ClaudePopup.ps1\\\"";
        ...
        Shown += async (_, _) =>
        {
            ...
            try
            {
                _webView2UserDataFolder = Path.Combine(Path.GetTempPath(), "ClaudePopup_Setup_" + Guid.NewGuid().ToString("N"));
    ...
    private InstallResult RunAutoInstall()
    {
        ...
        try
        {
            ...
            string ps1Path = Path.Combine(_hooksDir, "Show-ClaudePopup.ps1");
        ...
        if (hooksNode[eventName] is JsonArray existingArray)
        {
            ...
            foreach (var ruleSet in existingArray)
            {
                if (ruleSet?["hooks"] is JsonArray hooksArray)
                {
                    ...
                    {
                        if (hook?["command"]?.GetValue<string>()?.Contains("Show-ClaudePopup") == true)
                        {
                            alreadyExists = true;
                            break;
                        }
    ...
    ""Stop"": [
      {
        ""hooks"": [
          {
            ""type"": ""command"",
            ""command"": ""powershell.exe -ExecutionPolicy Bypass -File \""HOOKS_DIR_PLACEHOLDER\\Show-ClaudePopup.ps1\""""
          }
        ]
      }
    ],
...
<div class=""usage-grid"">
    <div class=""usage-card"">
        ...
        <div class=""vals"">Default: &quot;Claude Code&quot;</div>
    </div>
    <div class=""usage-card"">
